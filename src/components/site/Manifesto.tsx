...
export function Manifesto() {
  ...
  return (
    <section ref={ref} id="manifesto" className="relative overflow-hidden bg-[color:var(--forest-deep)] py-28 md:py-40 text-[color:var(--cream)]">
      ...
      />
      <div aria-hidden="true" className="absolute inset-0 bg-gradient-to-r from-[color:var(--forest-deep)] via-[color:var(--forest-deep)]/78 to-transparent" />
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[color:var(--gold)]/35" />
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 bottom-0 h-px bg-[color:var(--gold)]/35" />

      <div className="relative mx-auto grid max-w-[1280px] gap-16 px-6 md:grid-cols-[1.1fr_0.9fr] md:gap-24 md:px-12">
        <div>
          <Reveal as="span" className="inline-flex items-center gap-3 text-[10px] tracking-luxe uppercase text-[color:var(--gold)]/90">
            ...
            {WORDS.map((w, i) => (
              <motion.span
                ...
                className={`mr-[0.3em] inline-block transition-colors duration-500 ${
                  w.accent
                    ? "cursor-default hover:text-[color:var(--gold)]"
                    : ""
                }`}
              >
                {w.text}
        ...
        <Reveal delay={0.3} className="space-y-7 self-end font-display text-lg md:text-xl italic leading-[1.7] text-[color:var(--cream)]/92">
          <p>
            Cada peça que entra no acervo passa por um filtro humano — tecido, corte, origem,
            <span className="cursor-default not-italic transition-colors duration-500 hover:text-[color:var(--gold)]"> assinatura</span>.
            Recusamos volume. Honramos a
            <span className="cursor-default not-italic transition-colors duration-500 hover:text-[color:var(--gold)]"> escolha</span>.
          </p>
