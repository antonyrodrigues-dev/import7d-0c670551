...
BEGIN
  ...
  FOR v_chave, v_qty IN SELECT key, value::int FROM jsonb_each_text(v_agg) ORDER BY key LOOP
    ...
    IF v_size = '' THEN
      ...
      END IF;
      -- SOLD-OUT GUARD: peça sem tamanho confirmado só entra no funil se
      -- houver saldo físico disponível em ALGUMA variação. Esgotado (<=0)
      -- nunca gera reserva/pedido, mesmo com tamanho pendente.
      IF NOT EXISTS (
        SELECT 1 FROM public.produto_variacoes pv
         WHERE pv.produto_id = v_prod.id
           AND (CASE WHEN v_prod.modelo_estoque = 'kit'
                     THEN public.kit_disponivel(v_prod.id, pv.tamanho)
                     ELSE GREATEST(COALESCE(pv.disponivel,0),0) END) > 0
      ) THEN
        RAISE EXCEPTION 'Peça esgotada: % não pode ser reservada.', v_slug
          USING ERRCODE = '22023';
      END IF;

      -- FINAL-A/A1 — PEÇA ÚNICA COM TAMANHO PÚBLICO PENDENTE.
      -- A unidade física existe e é UMA só: precisa de hold atômico agora,
      -- senão dois clientes reservam a mesma peça. O tamanho público do
      -- pedido continua NULL (nunca inventamos tamanho para o cliente);
      -- a reserva é gravada na variação física real.
      -- Multi-variante NÃO recebe hold aqui: sem a variante não há unidade
      -- identificável — segue apenas como intenção/atendimento.
      IF v_prod.modelo_estoque = 'peca_unica' THEN
        FOR v_comp IN SELECT pv.tamanho FROM public.produto_variacoes pv
                       WHERE pv.produto_id = v_prod.id ORDER BY pv.tamanho LOOP
          PERFORM public.expirar_reservas_variacao(v_prod.id, v_comp.tamanho);
        END LOOP;
        -- Trava TODAS as variações da peça: o concorrente espera aqui e só
        -- então relê o saldo já debitado pelo primeiro hold.
        PERFORM 1 FROM public.produto_variacoes pv
          WHERE pv.produto_id = v_prod.id ORDER BY pv.tamanho FOR UPDATE;
        SELECT pv.tamanho AS tamanho INTO v_var
          FROM public.produto_variacoes pv
         WHERE pv.produto_id = v_prod.id
           AND GREATEST(COALESCE(pv.disponivel,0),0) >= v_qty
         ORDER BY pv.tamanho
         LIMIT 1;
        IF NOT FOUND THEN
          RAISE EXCEPTION 'Peça esgotada: % não pode ser reservada.', v_slug
        ...
        v_reservas := v_reservas || jsonb_build_array(jsonb_build_object(
          'produto_id', v_prod.id, 'tamanho', v_var.tamanho, 'quantidade', v_qty));
      END IF;
      v_pend_tam := true;
...
DECLARE
  ...
  v_hash text := encode(extensions.digest(p_telefone, 'sha256'), 'hex');
BEGIN
  ...
  SELECT COALESCE(sum(r.quantidade), 0) INTO v_reservas
   ...
   WHERE r.estado IN ('reservada_temporariamente','em_atendimento')
     AND r.expira_em > now()
...
CREATE OR REPLACE FUNCTION public.resolver_pendencias_pedido(p_pedido_id uuid, p_itens jsonb, p_motivo_preco text DEFAULT NULL::text)
 RETURNS pedidos
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
    ...
    IF v_prod.modelo_estoque <> 'kit' AND NOT EXISTS (
      SELECT 1 FROM public.reservas_estoque r
       WHERE r.pedido_id = v_pedido.id AND r.produto_id = v_prod.id
         AND r.tamanho = v_size AND r.estado IN ('reservada_temporariamente','em_atendimento')
    ) THEN
      PERFORM public.expirar_reservas_variacao(v_prod.id, v_size);
...
DROP FUNCTION IF EXISTS public.resolver_pendencias_pedido(uuid, jsonb);

CREATE OR REPLACE VIEW public.catalogo_publico AS
 SELECT slug, nome, categoria, colecao, cor, marca, descricao, imagens,
    CASE WHEN preco_status = 'confirmado' AND COALESCE(preco,0) > 0 THEN preco END AS preco,
    CASE WHEN preco_status = 'confirmado' AND COALESCE(preco,0) > 0 THEN preco_cartao END AS preco_cartao,
    CASE WHEN preco_status = 'confirmado' AND COALESCE(preco,0) > 0 THEN parcelamento END AS parcelamento,
    (preco_status = 'confirmado' AND COALESCE(preco,0) > 0) AS preco_confirmado,
    destaque, modelo_estoque, criado_em,
      ...
      SELECT 1 FROM public.produto_variacoes v
       WHERE v.produto_id = p.id
         ...
         AND (CASE WHEN p.modelo_estoque = 'kit' THEN public.kit_disponivel(p.id, v.tamanho)
                   ELSE GREATEST(COALESCE(v.disponivel,0),0) END) > 0) AS compravel,
    EXISTS (
      SELECT 1 FROM public.produto_variacoes v
       WHERE v.produto_id = p.id
         AND v.origem_tamanho IN ('confirmado_etiqueta','confirmado_medicao')
         AND (CASE WHEN p.modelo_estoque = 'kit' THEN public.kit_disponivel(p.id, v.tamanho)
                   ELSE GREATEST(COALESCE(v.disponivel,0),0) END) > 0) AS tamanho_confirmado,
    (ativo AND arquivado_em IS NULL AND COALESCE((
       SELECT sum(CASE WHEN p.modelo_estoque = 'kit'
                       THEN public.kit_disponivel(p.id, v.tamanho)
                       ELSE GREATEST(COALESCE(v.disponivel,0),0) END)
         FROM public.produto_variacoes v WHERE v.produto_id = p.id), 0) > 0) AS reservavel,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('tamanho', v.tamanho, 'disponivel',
                CASE WHEN p.modelo_estoque = 'kit' THEN public.kit_disponivel(p.id, v.tamanho)
                     ELSE GREATEST(COALESCE(v.disponivel,0),0) END) ORDER BY v.tamanho)
           FROM public.produto_variacoes v
          WHERE v.produto_id = p.id
            AND v.origem_tamanho IN ('confirmado_etiqueta','confirmado_medicao')
            AND (CASE WHEN p.modelo_estoque = 'kit' THEN public.kit_disponivel(p.id, v.tamanho)
                      ELSE GREATEST(COALESCE(v.disponivel,0),0) END) > 0), '[]'::jsonb) AS variacoes,
    -- FINAL-A/A2 — saldo físico REAL (todas as variações, com ou sem tamanho
    -- confirmado, já descontados holds e quarentena). Autoridade de
    -- reservabilidade da vitrine: 0 = Indisponível, nunca entra na Reserva.
    COALESCE((SELECT sum(CASE WHEN p.modelo_estoque = 'kit'
                              THEN public.kit_disponivel(p.id, v.tamanho)
                              ELSE GREATEST(COALESCE(v.disponivel,0),0) END)
                FROM public.produto_variacoes v WHERE v.produto_id = p.id), 0)::int AS saldo_fisico
   FROM public.produtos p
  WHERE arquivado_em IS NULL
    AND jsonb_array_length(COALESCE(imagens,'[]'::jsonb)) > 0
